...
function hasAnyVital(v: MedMalDocumentEvent["vitals"]): boolean {
    if (!v) return false;
    return (
        v.bp != null ||
        v.hr != null ||
        ...
        v.temp_c != null ||
        v.map != null ||
        v.urine_output_ml != null
    );
}

function fmtNum(n: number | null | undefined): string {
    if (n == null || !Number.isFinite(n)) return "—";
    // Round to 1 decimal for floats, integer otherwise.
    return Number.isInteger(n) ? String(n) : n.toFixed(1);
}

function ProjectVitalsPage({
    projectId,
    docId,
}: {
    projectId: string;
    ...
    if (project && project.template_id !== "med-mal-case") {
        return (
            <div className="max-w-lg mx-auto py-16 px-4 text-center text-gray-600">
                <p className="mb-4">
                    The vitals trend view is available for{" "}
            ...
            <div className="flex items-center justify-between border-b border-gray-200 px-4 py-2 shrink-0">
                <div>
                    ...
                    <p className="text-xs text-gray-500">
                        {project?.name} · {doc.filename} · {rows.length} entries
                    </p>
                </div>
...
export default function Page({
    ...
    return <ProjectVitalsPage projectId={id} docId={docId} />;
}
