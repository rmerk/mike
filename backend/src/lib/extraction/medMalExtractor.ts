...
import { insertDocumentEvents, type DocumentEventInsert } from "./eventLog";
...
- Never use "peer_review_145_64" (that path is server-side only).`;
...
function normalizeLlmEvents(
    ...
    for (const row of evs) {
        ...
        if (typeof sp !== "number" || sp !== pageNum) continue;
        const bboxRaw = o.source_bbox;
        if (!bboxRaw || typeof bboxRaw !== "object") continue;
        ...
        if (source_bbox.w <= 0 || source_bbox.h <= 0) continue;
        const privacy =
            typeof o.privacy_class === "string" ? o.privacy_class : "standard";
        const safePrivacy =
            privacy === "mental_health_144_293" ? privacy : "standard";
        out.push({
            document_id: documentId,
            extraction_run_id: runId,
            event_date: (o.event_date as string) ?? null,
            event_time: (o.event_time as string) ?? null,
            event_date_text: (o.event_date_text as string) ?? null,
            provider: (o.provider as string) ?? null,
            provider_role: (o.provider_role as string) ?? null,
            episode_of_care: (o.episode_of_care as string) ?? null,
            encounter_type: (o.encounter_type as string) ?? null,
            privacy_class: safePrivacy,
            key_date_role: (o.key_date_role as string) ?? null,
...
export async function executeMedMalExtraction(params: {
    ...
    try {
        ...
        if (peerPages.length > 0) {
            await db.from("document_red_flags").insert({
                document_id: documentId,
                extraction_run_id: runId,
        ...
        for (let pageNum = 1; pageNum <= numPages; pageNum++) {
            ...
            try {
                ...
                if (toInsert.length) await insertDocumentEvents(db, toInsert);
